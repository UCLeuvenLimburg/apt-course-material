...
[source,csharp]
----
var minorCount = Count( Map( Filter(people, p => p.Gender == Genders.Male ), p => p.Age ), age => age < 18 );
----
...
[source,ruby]
----
minorCount = people.select { |p| p.gender == Genders.male }
                   .map    { |p| p.age }
                   .count  { |n| n < 18 }
----
...
In fact, we could say that there exist a world of programming concepts independent of languages, such as functions, classes, modules, etc.
However, no language supports every one of these concepts.
A design pattern is then a "projection" of the concept onto the language.
In Java, for example, Strategy explains how functions can be built out of classes.
Likewise, the complex Visitor design pattern describes how to achieve multiple dispatch by combining classes, etc.

The programming language C does not support classes.
It is possible though to built them out of structs and function pointers, both supported in C.
